...
> Ch08 给了我们理论：后验 $\propto$ 似然 $\times$ 先验——每一条新数据流入，我们对 $\theta$ 的认知熵就减少一点，后验分布比先验更"集中"。但这个熵减过程在数学上是优雅的积分，在计算上却是噩梦——除共轭族外，后验几乎永远没有解析形式。本章的任务，是把贝叶斯定理从纸面上的数学陈述变成计算机里的可执行代码。如果说 Ch08 是贝叶斯理论的"宪法"，本章就是它的"工程实施条例"——MCMC、变分推断、重要性抽样，都是为了让认知熵减从承诺变为现实。
