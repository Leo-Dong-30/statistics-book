...
> 从信息论视角看，单个学习器可能陷入局部最优（"困在自己的认知偏差中"），而集成通过聚合多个"有偏但不同"的视角，降低了整体的不确定性。Bagging 的 Bootstrap 抽样制造了差异化的训练集，让每个学习器看到略有不同的"版本的世界"——这增加了单一学习器的方差，但通过平均抵消了方差。**集成的艺术，就是在制造差异和消除差异之间找到最优平衡。**
